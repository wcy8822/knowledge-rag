...
- [ ] BM25 索引增量更新：pipeline 新增文档后需 rebuild（手动跑 `python3 code/scripts/loki_bm25_index.py`）
- [ ] 将 BM25 rebuild 集成到 loki_pipeline.py 自动流程
- [ ] Benchmark 改造为 QA 评测（给 AI 返回结果，问 AI 能否答对）—— 当前 recall 指标只测文件名匹配
