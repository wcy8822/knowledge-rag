...
def main():
    ...
    with open(VECTOR_DB_FILE, 'w', encoding='utf-8') as f:
        ...
        f.write('  "vector_dim": ' + str(VECTOR_DIM) + ',\n')
        f.write('  "vector_db": [\n')
        ...
        for batch_start in range(0, len(MD_FILES), BATCH_SIZE):
            ...
            for i, file_path in enumerate(batch_files, batch_start+1):
                ...
                for j, chunk in enumerate(chunks):
                    ...
                    f.write(f'      "chunk_text": "{chunk.replace(chr(10), r"\\n")}",\n')
                    f.write(f'      "vector": {vector}\n')
                    f.write('    }')
                    
                    # 如果不是最后一个chunk，或者是最后一个批次中的最后一个文件的最后一个chunk，就加逗号
                    if not (j == len(chunks) - 1 and i == batch_end - 1):
                        f.write(',\n')
                    else:
                        f.write('\n')
        ...
        # 写入向量库的结尾
        f.write('  ],\n')
        f.write(f'  "process_time": "{(time.time() - start_time):.2f} 秒"\n')
        f.write('}\n')
