...
def setup_logger(
    ...
    # 创建格式化器（只创建一次）
    if not hasattr(setup_logger, "_formatter"):
        if json_format:
            setup_logger._formatter = logging.Formatter('{"timestamp": "%(asctime)s", "name": "%(name)s", "level": "%(levelname)s", "message": "%(message)s"}')
        else:
            setup_logger._formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
    formatter = setup_logger._formatter

    # 控制台处理器（只创建一次，使用 WARNING 级别减少输出）
    if console_output:
        if "console_handler" not in setup_logger._handler_cache:
    ...
    # 文件处理器（只创建一次，使用 WARNING 级别减少 I/O）
    if file_output and "file_handler" not in setup_logger._handler_cache:
        ...
        try:
            ...
            setup_logger._handler_cache["file_handler"] = file_handler
        except Exception:
            pass  # 静默失败，不影响主程序

    # 缓存 logger
    setup_logger._logger_cache[name] = logger
